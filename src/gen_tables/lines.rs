			...
			unsafe {
				LINE[src.to_index()][dest.to_index()] = ALL_SQUARES
					...
					.filter(|test| {
						...
						if (src_rank - dest_rank).abs() == (src_file - dest_file).abs() && *src != *dest {
							(src_rank - test_rank).abs() == (src_file - test_file).abs() && (dest_rank - test_rank).abs() == (dest_file - test_file).abs()
						// next, test rank/file lines
						} else if (src_rank == dest_rank || src_file == dest_file) && *src != *dest {
							(src_rank == test_rank && dest_rank == test_rank) || (src_file == test_file && dest_file == test_file)
...
pub fn write_lines(f: &mut File) {
	writeln!(f, "static LINE: [[BitBoard; 64]; 64] = [[").unwrap();
	let l = unsafe { LINE };
