	...
	for color in ALL_COLORS.iter() {
		unsafe {
			KINGSIDE_CASTLE_SQUARES[color.to_index()] =
				BitBoard::set(color.to_my_backrank(), ChessFile::F) ^ BitBoard::set(color.to_my_backrank(), ChessFile::G);
		}
	}
}
...
fn gen_queenside_castle_squares() {
	for color in ALL_COLORS.iter() {
		unsafe {
			QUEENSIDE_CASTLE_SQUARES[color.to_index()] = BitBoard::set(color.to_my_backrank(), ChessFile::B)
...
pub fn write_king_moves(f: &mut File) {
	...
	unsafe { writeln!(f, " BitBoard({}), BitBoard({})];", KINGSIDE_CASTLE_SQUARES[0].0, KINGSIDE_CASTLE_SQUARES[1].0).unwrap() };

	writeln!(f, "pub const QUEENSIDE_CASTLE_SQUARES: [BitBoard; 2] = [").unwrap();
	unsafe { writeln!(f, " BitBoard({}), BitBoard({})];", QUEENSIDE_CASTLE_SQUARES[0].0, QUEENSIDE_CASTLE_SQUARES[1].0).unwrap() };

	writeln!(f, "const CASTLE_MOVES: BitBoard = BitBoard({});", gen_castle_moves().0).unwrap();
}
