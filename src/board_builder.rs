...
impl BoardBuilder {
	...
	/// # }
	pub fn setup<'a>(
		pieces: impl IntoIterator<Item = &'a (Square, Piece, Color)>, side_to_move: Color, white_castle_rights: CastleRights,
		black_castle_rights: CastleRights, en_passant: Option<File>,
	) -> BoardBuilder {
		let mut result = BoardBuilder { pieces: [None; 64], side_to_move, castle_rights: [white_castle_rights, black_castle_rights], en_passant };
	...
	pub fn get_en_passant(&self) -> Option<Square> {
		self.en_passant.map(|f| Square::make_square((!self.get_side_to_move()).to_fourth_rank(), f))
	}
...
impl fmt::Display for BoardBuilder {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		...
		write!(f, "{}", self.castle_rights[Color::White.to_index()].to_string(Color::White))?;
		write!(f, "{}", self.castle_rights[Color::Black.to_index()].to_string(Color::Black))?;
		if self.castle_rights[0] == CastleRights::NoRights && self.castle_rights[1] == CastleRights::NoRights {
			write!(f, "-")?;
		}
	...
	fn from_str(value: &str) -> Result<Self, Self::Err> {
		...
		for x in pieces.chars() {
			match x {
				...
				'r' => {
					fen[Square::make_square(cur_rank, cur_file)] = Some((Piece::Rook, Color::Black));
					cur_file = cur_file.right();
				}
				'R' => {
					fen[Square::make_square(cur_rank, cur_file)] = Some((Piece::Rook, Color::White));
					cur_file = cur_file.right();
				}
				'n' => {
					fen[Square::make_square(cur_rank, cur_file)] = Some((Piece::Knight, Color::Black));
					cur_file = cur_file.right();
				}
				'N' => {
					fen[Square::make_square(cur_rank, cur_file)] = Some((Piece::Knight, Color::White));
					cur_file = cur_file.right();
				}
				'b' => {
					fen[Square::make_square(cur_rank, cur_file)] = Some((Piece::Bishop, Color::Black));
					cur_file = cur_file.right();
				}
				'B' => {
					fen[Square::make_square(cur_rank, cur_file)] = Some((Piece::Bishop, Color::White));
					cur_file = cur_file.right();
				}
				'p' => {
					fen[Square::make_square(cur_rank, cur_file)] = Some((Piece::Pawn, Color::Black));
					cur_file = cur_file.right();
				}
				'P' => {
					fen[Square::make_square(cur_rank, cur_file)] = Some((Piece::Pawn, Color::White));
					cur_file = cur_file.right();
				}
				'q' => {
					fen[Square::make_square(cur_rank, cur_file)] = Some((Piece::Queen, Color::Black));
					cur_file = cur_file.right();
				}
				'Q' => {
					fen[Square::make_square(cur_rank, cur_file)] = Some((Piece::Queen, Color::White));
					cur_file = cur_file.right();
				}
				'k' => {
					fen[Square::make_square(cur_rank, cur_file)] = Some((Piece::King, Color::Black));
					cur_file = cur_file.right();
				}
				'K' => {
					fen[Square::make_square(cur_rank, cur_file)] = Some((Piece::King, Color::White));
					cur_file = cur_file.right();
				}
				_ => {
					return Err(Error::InvalidFen { fen: value.to_string() });
		...
		match side {
			...
			_ => return Err(Error::InvalidFen { fen: value.to_string() }),
		}
...
fn test_kissing_kings() {
	let res: Result<Board, _> = BoardBuilder::new().piece(Square::A1, Piece::King, Color::White).piece(Square::A2, Piece::King, Color::Black).try_into();
	assert!(res.is_err());
}
