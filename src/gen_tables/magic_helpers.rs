...
pub fn random_bitboard<R: RngExt>(rng: &mut R) -> BitBoard {
	...
	get_rays(sq, piece)
		& if piece == Piece::Bishop {
			...
			!ALL_SQUARES
				.iter()
				.filter(|edge| {
					(sq.get_rank() == edge.get_rank() && (edge.get_file() == File::A || edge.get_file() == File::H))
						|| (sq.get_file() == edge.get_file() && (edge.get_rank() == Rank::First || edge.get_rank() == Rank::Eighth))
				})
...
fn gen_edges() -> BitBoard {
	ALL_SQUARES
		.iter()
		.filter(|sq| sq.get_rank() == Rank::First || sq.get_rank() == Rank::Eighth || sq.get_file() == File::A || sq.get_file() == File::H)
		.fold(EMPTY, |b, s| b | BitBoard::from_square(*s))
}
