...
use crate::piece::{NUM_PROMOTION_PIECES, PROMOTION_PIECES, Piece};
...
impl SquareAndBitBoard {
	pub fn new(sq: Square, bb: BitBoard, promotion: bool) -> SquareAndBitBoard {
		SquareAndBitBoard { square: sq, bitboard: bb, promotion }
	}
}
...
impl MoveGen {
	...
	pub fn new_legal(board: &Board) -> MoveGen {
		MoveGen { moves: MoveGen::enumerate_moves(board), promotion_index: 0, iterator_mask: !EMPTY, index: 0 }
	}
	...
	pub fn legal_quick(board: &Board, chess_move: ChessMove) -> bool {
		...
		match piece {
			...
			Piece::Queen => true,
			Piece::Pawn => {
				if chess_move.get_source().get_file() != chess_move.get_dest().get_file() && board.piece_on(chess_move.get_dest()).is_none() {
					// en-passant
...
impl Iterator for MoveGen {
	...
	fn next(&mut self) -> Option<ChessMove> {
		if self.index >= self.moves.len() || self.moves[self.index].bitboard & self.iterator_mask == EMPTY {
			// are we done?
			None
			...
			let result = ChessMove::new(moves.square, dest, Some(PROMOTION_PIECES[self.promotion_index]));
...
#[test]
fn movegen_issue_15() {
	let board = BoardBuilder::from_str("rnbqkbnr/ppp2pp1/4p3/3N4/3PpPp1/8/PPP3PP/R1B1KBNR b KQkq f3 0 1").unwrap().try_into().unwrap();
...
fn move_of(m: &str) -> ChessMove {
	...
	ChessMove::new(Square::from_str(&m[..2]).unwrap(), Square::from_str(&m[2..4]).unwrap(), promo)
}
...
fn test_masked_move_gen() {
	...
	let expected = vec![move_of("f4e5"), move_of("b3d5"), move_of("g5e4"), move_of("g5f7"), move_of("g5h7"), move_of("c3e4"), move_of("c3d5")];
