...
use crate::magic::{
	between, get_adjacent_files, get_bishop_moves, get_bishop_rays, get_king_moves, get_knight_moves, get_pawn_attacks, get_pawn_moves, get_rank,
	get_rook_moves, get_rook_rays, line,
};
...
pub trait PieceType {
		...
		T: CheckType,
	{
		...
		let check_mask = if T::IN_CHECK { between(checkers.to_square(), ksq) ^ checkers } else { !EMPTY };
...
impl PawnType {
	...
	pub fn legal_ep_move(board: &Board, source: Square, dest: Square) -> bool {
		let combined = board.combined() ^ BitBoard::from_square(board.en_passant().unwrap()) ^ BitBoard::from_square(source) ^ BitBoard::from_square(dest);

		let ksq = (board.pieces(Piece::King) & board.color_combined(board.side_to_move())).to_square();

		let rooks = (board.pieces(Piece::Rook) | board.pieces(Piece::Queen)) & board.color_combined(!board.side_to_move());

		if (get_rook_rays(ksq) & rooks) != EMPTY && (get_rook_moves(ksq, combined) & rooks) != EMPTY {
			return false;
		}

		let bishops = (board.pieces(Piece::Bishop) | board.pieces(Piece::Queen)) & board.color_combined(!board.side_to_move());
...
impl PieceType for PawnType {
		...
		T: CheckType,
	{
		...
		for src in pieces & !pinned {
			...
			if moves != EMPTY {
				unsafe {
					movelist.push_unchecked(SquareAndBitBoard::new(src, moves, src.get_rank() == color.to_seventh_rank()));
				}
			}
		}
		...
		if board.en_passant().is_some() {
			...
			for src in rank & files & pieces {
				...
				if PawnType::legal_ep_move(board, src, dest) {
					unsafe {
						movelist.push_unchecked(SquareAndBitBoard::new(src, BitBoard::from_square(dest), false));
					}
				}
			}
		}
...
impl KingType {
	...
	pub fn legal_king_move(board: &Board, dest: Square) -> bool {
		let combined = board.combined() ^ (board.pieces(Piece::King) & board.color_combined(board.side_to_move())) | BitBoard::from_square(dest);
		...
		attackers |= knight_rays & board.pieces(Piece::Knight) & board.color_combined(!board.side_to_move());

		let king_rays = get_king_moves(dest);
		attackers |= king_rays & board.pieces(Piece::King) & board.color_combined(!board.side_to_move());

		attackers |= get_pawn_attacks(dest, board.side_to_move(), board.pieces(Piece::Pawn) & board.color_combined(!board.side_to_move()));
...
impl PieceType for KingType {
		...
		T: CheckType,
	{
		...
		if !T::IN_CHECK {
			if board.my_castle_rights().has_kingside() && (combined & board.my_castle_rights().kingside_squares(color)) == EMPTY {
				let middle = ksq.uright();
				let right = middle.uright();
				if KingType::legal_king_move(board, middle) && KingType::legal_king_move(board, right) {
					moves ^= BitBoard::from_square(right);
				}
			}

			if board.my_castle_rights().has_queenside() && (combined & board.my_castle_rights().queenside_squares(color)) == EMPTY {
				let middle = ksq.uleft();
				let left = middle.uleft();
				if KingType::legal_king_move(board, middle) && KingType::legal_king_move(board, left) {
