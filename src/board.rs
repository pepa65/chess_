...
use crate::color::{ALL_COLORS, Color, NUM_COLORS};
use crate::error::Error;
use crate::file::File;
use crate::magic::{
	between, get_adjacent_files, get_bishop_rays, get_castle_moves, get_file, get_king_moves, get_knight_moves, get_pawn_attacks, get_pawn_dest_double_moves,
	get_pawn_source_double_moves, get_rank, get_rook_rays,
};
use crate::movegen::*;
use crate::piece::{ALL_PIECES, NUM_PIECES, Piece};
...
impl Default for Board {
	#[inline]
	fn default() -> Board {
		Board::from_str("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1").expect("Valid Position")
	}
}
...
impl Board {
	...
	pub fn is_sane(&self) -> bool {
		...
		if let Some(x) = self.en_passant
			&& self.pieces(Piece::Pawn) & self.color_combined(!self.side_to_move) & BitBoard::from_square(x) == EMPTY
		{
			return false;
		}
		...
		for color in ALL_COLORS.iter() {
			...
			if castle_rights.unmoved_rooks(*color) & self.pieces(Piece::Rook) & self.color_combined(*color) != castle_rights.unmoved_rooks(*color) {
				return false;
			}
			// if we have castle rights, make sure we have a king on the (E, {1,8}) square,
			// depending on the color
			if castle_rights != CastleRights::NoRights
				&& self.pieces(Piece::King) & self.color_combined(*color) != get_file(File::E) & get_rank(color.to_my_backrank())
			{
				return false;
			}
		}
	...
	pub fn get_hash(&self) -> u64 {
		self.hash
			^ if let Some(ep) = self.en_passant { Zobrist::en_passant(ep.get_file(), !self.side_to_move) } else { 0 }
			^ Zobrist::castles(self.castle_rights[self.side_to_move.to_index()], self.side_to_move)
			^ Zobrist::castles(self.castle_rights[(!self.side_to_move).to_index()], !self.side_to_move)
			^ if self.side_to_move == Color::Black { Zobrist::color() } else { 0 }
	}
	...
	pub fn piece_on(&self, square: Square) -> Option<Piece> {
		...
		if self.combined() & opp == EMPTY {
			...
			if (self.pieces(Piece::Pawn) ^ self.pieces(Piece::Knight) ^ self.pieces(Piece::Bishop)) & opp != EMPTY {
	...
	fn set_ep(&mut self, sq: Square) {
		...
		if get_adjacent_files(sq.get_file()) & get_rank(sq.get_rank()) & self.pieces(Piece::Pawn) & self.color_combined(!self.side_to_move) != EMPTY {
	...
	pub fn make_move_new(&self, m: ChessMove) -> Board {
		...
		#[allow(deprecated)]
		result.remove_their_castle_rights(CastleRights::square_to_castle_rights(!self.side_to_move, dest));

		#[allow(deprecated)]
		result.remove_my_castle_rights(CastleRights::square_to_castle_rights(self.side_to_move, source));
		...
		const CASTLE_ROOK_START: [File; 8] = [File::A, File::A, File::A, File::A, File::H, File::H, File::H, File::H];
		const CASTLE_ROOK_END: [File; 8] = [File::D, File::D, File::D, File::D, File::F, File::F, File::F, File::F];
		...
		if moved == Piece::Knight {
			...
			if let Some(Piece::Knight) = m.get_promotion() {
			...
			} else if (source_bb & get_pawn_source_double_moves()) != EMPTY && (dest_bb & get_pawn_dest_double_moves()) != EMPTY {
				result.set_ep(dest);
				...
				result.xor(Piece::Pawn, BitBoard::from_square(dest.ubackward(self.side_to_move)), !self.side_to_move);
				result.checkers ^= get_pawn_attacks(ksq, !result.side_to_move, dest_bb);
			} else {
				result.checkers ^= get_pawn_attacks(ksq, !result.side_to_move, dest_bb);
			}
		...
		let attackers = result.color_combined(result.side_to_move)
			& ((get_bishop_rays(ksq) & (result.pieces(Piece::Bishop) | result.pieces(Piece::Queen)))
				| (get_rook_rays(ksq) & (result.pieces(Piece::Rook) | result.pieces(Piece::Queen))));
	...
	fn update_pin_info(&mut self) {
		...
		self.checkers ^= get_knight_moves(ksq) & self.color_combined(!self.side_to_move) & self.pieces(Piece::Knight);

		self.checkers ^= get_pawn_attacks(ksq, self.side_to_move, self.color_combined(!self.side_to_move) & self.pieces(Piece::Pawn));
	}
...
impl TryFrom<&BoardBuilder> for Board {
	...
	fn try_from(fen: &BoardBuilder) -> Result<Self, Self::Error> {
		...
		if board.is_sane() { Ok(board) } else { Err(Error::InvalidBoard) }
	}
}
...
fn test_null_move_en_passant() {
	let start = Board::from_str("rnbqkbnr/pppp2pp/8/4pP2/8/8/PPPP1PPP/RNBQKBNR w KQkq e6 0 0").unwrap();
	let expected = Board::from_str("rnbqkbnr/pppp2pp/8/4pP2/8/8/PPPP1PPP/RNBQKBNR b KQkq - 0 0").unwrap();
