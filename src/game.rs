...
impl Game {
	...
	/// ```
	pub fn new() -> Game {
		Game { start_pos: Board::default(), moves: vec![] }
	}
	...
	pub fn new_with_board(board: Board) -> Game {
		Game { start_pos: board, moves: vec![] }
	}
	...
	pub fn can_declare_draw(&self) -> bool {
		...
		for x in self.moves.iter() {
			if let Action::MakeMove(m) = *x {
				...
				if board.piece_on(m.get_source()) == Some(Piece::Pawn) || board.piece_on(m.get_dest()).is_some() {
					reversible_moves = 0;
				...
				if board.castle_rights(Color::White) != white_castle_rights || board.castle_rights(Color::Black) != black_castle_rights {
					reversible_moves = 0;
					legal_moves_per_turn.clear();
				}
				legal_moves_per_turn.push((board.get_hash(), MoveGen::new_legal(&board).collect()));
	...
	pub fn side_to_move(&self) -> Color {
		let move_count =
			self.moves.iter().filter(|m| matches!(*m, Action::MakeMove(_))).count() + if self.start_pos.side_to_move() == Color::White { 0 } else { 1 };

		if move_count % 2 == 0 { Color::White } else { Color::Black }
	}
	...
	pub fn accept_draw(&mut self) -> bool {
		...
		if !self.moves.is_empty()
			&& (self.moves[self.moves.len() - 1] == Action::OfferDraw(Color::White) || self.moves[self.moves.len() - 1] == Action::OfferDraw(Color::Black))
		{
...
pub fn fake_pgn_parser(moves: &str) -> Game {
	moves.split_whitespace().filter(|s| !s.ends_with(".")).fold(Game::new(), |mut g, m| {
