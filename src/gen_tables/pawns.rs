			...
			unsafe {
				if src.get_rank() == color.to_second_rank() {
					PAWN_MOVES[color.to_index()][src.to_index()] =
						BitBoard::from_square(src.uforward(*color)) ^ BitBoard::from_square(src.uforward(*color).uforward(*color));
				} else {
...
pub fn write_pawn_moves(f: &mut File) {
	...
	let pm = unsafe { PAWN_MOVES };
	for (i, row) in pm.iter().enumerate() {
		for j in row {
			writeln!(f, "    BitBoard({}),", j.0).unwrap();
		}
		if i != 1 {
			writeln!(f, "  ], [").unwrap();
		}
...
pub fn write_pawn_attacks(f: &mut File) {
	...
	let pa = unsafe { PAWN_ATTACKS };
	for (i, row) in pa.iter().enumerate() {
		for j in row {
			writeln!(f, "    BitBoard({}),", j.0).unwrap();
		}
		if i != 1 {
	...
	writeln!(f, "const PAWN_SOURCE_DOUBLE_MOVES: BitBoard = BitBoard({0});", gen_source_double_moves().0).unwrap();

	writeln!(f, "const PAWN_DEST_DOUBLE_MOVES: BitBoard = BitBoard({0});", gen_dest_double_moves().0).unwrap();
}
