...
pub fn get_rook_moves_bmi(sq: Square, blockers: BitBoard) -> BitBoard {
	unsafe {
		...
		let index = (_pext_u64(blockers.0, bmi2_magic.blockers_mask.0) as usize) + (bmi2_magic.offset as usize);
...
pub fn get_bishop_moves_bmi(sq: Square, blockers: BitBoard) -> BitBoard {
	unsafe {
		...
		let index = (_pext_u64(blockers.0, bmi2_magic.blockers_mask.0) as usize) + (bmi2_magic.offset as usize);
		let result = _pdep_u64(*BMI_MOVES.get_unchecked(index as usize) as u64, get_bishop_rays(sq).0);
		return BitBoard(result);
	}
}
...
pub fn get_pawn_attacks(sq: Square, color: Color, blockers: BitBoard) -> BitBoard {
	unsafe { *PAWN_ATTACKS.get_unchecked(color.to_index()).get_unchecked(sq.to_index()) & blockers }
}
...
pub fn line(sq1: Square, sq2: Square) -> BitBoard {
	unsafe { *LINE.get_unchecked(sq1.to_index()).get_unchecked(sq2.to_index()) }
}

/// Get a line between these two squares, not including the squares themselves.
#[inline]
pub fn between(sq1: Square, sq2: Square) -> BitBoard {
	unsafe { *BETWEEN.get_unchecked(sq1.to_index()).get_unchecked(sq2.to_index()) }
}
