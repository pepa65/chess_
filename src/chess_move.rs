...
impl ChessMove {
	...
	pub fn new(source: Square, dest: Square, promotion: Option<Piece>) -> ChessMove {
		ChessMove { source, dest, promotion }
	}
	...
	pub fn from_san(board: &Board, move_text: &str) -> Result<ChessMove, Error> {
		...
		let takes = move_text.get(cur_index..cur_index + 1).map(|s| s == "x").unwrap_or(false);
		if takes {
			cur_index += 1;
		}
		...
		let ep = if let Some(s) = move_text.get(cur_index..) { s == " e.p." } else { false };
		...
		for m in &mut MoveGen::new_legal(board) {
			...
			if let Some(rank) = source_rank
				&& m.get_source().get_rank() != rank
			{
				continue;
			}

			if let Some(file) = source_file
				&& m.get_source().get_file() != file
			{
				continue;
			}
